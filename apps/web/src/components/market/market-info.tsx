...
export function MarketInfo({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-4">
      ...
      {resolutionCriteria && (
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">{resolutionCriteria}</p>
          </div>
        </div>
      )}

      {/* Normalized Question */}
      {normalizedQuestion && normalizedQuestion !== resolutionCriteria && (
        <div className="flex items-start gap-3">
          <div className="p-2 rounded-lg bg-primary/10">
            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Stats */}
      {typeof totalPostsProcessed === "number" && totalPostsProcessed > 0 && (
        <div className="flex items-center gap-3 pt-2 border-t border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
            </svg>
            <span>{totalPostsProcessed.toLocaleString()} posts analyzed</span>
          </div>
        </div>
      )}
    </div>
